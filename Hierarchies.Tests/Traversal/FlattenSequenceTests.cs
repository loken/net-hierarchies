...
public class FlattenSequenceTests
{
	[Fact]
	public void SequenceNext_YieldsInOrder()
	{
		...
		var elements = Flatten.Sequence(list.First!, el => el.Next);
	...
	public void SequenceSignal_WithSkip_YieldsCorrectValues()
	{
		...
		var elements = Flatten.Sequence(list.First!, (el, signal) =>
		{
			// Skip odd numbers.
			if (el.Value % 2 == 1)
				signal.Skip();
	...
	public void SequenceSignal_WithSkip_ProvidesCorrectIndexAndCount()
	{
		...
		var actual = Flatten.Sequence(list.First, (el, signal) =>
		{
	...
	{
		var elements = Flatten.Sequence<LinkedListNode<int>>(null, el => el.Next);
	...
	{
		var elements = Flatten.Sequence<LinkedListNode<int>>(null, (el, signal) =>
		{
